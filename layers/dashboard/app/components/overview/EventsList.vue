    ...
    <ul class="divide-muted divide-y">
      ...
      >
        <div class="flex w-10 shrink-0 flex-col items-center text-center">
          <span class="text-default text-xl leading-none font-bold">{{
            event.day
          }}</span>
          <span
            class="text-2xs text-dimmed font-mono tracking-widest uppercase"
            >{{ event.month }}</span
          >
        </div>
        <div class="min-w-0 flex-1">
          <span
            class="text-2xs tracking-loose mb-1 inline-flex items-center rounded px-1.5 py-0.5 font-mono font-semibold uppercase"
            :class="badgeClass[event.type]"
          >
            {{ event.type }}
          </span>
          <p class="text-body-sm text-default font-medium">{{ event.title }}</p>
